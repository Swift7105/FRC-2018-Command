...
public class EncoderLeft extends Subsystem {
	...
	public void DriveLeftPID(double target, double power) {
		double error = (target * 1040) - Left.getDistance();
		if (error > 5) {
			RobotMap.LeftDriveBack.set(power);
			RobotMap.LeftDriveFront.set(power);
			while (error > 100) {
